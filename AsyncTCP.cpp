...
#include "..\my_setting/Parameters.h"
...
static void _async_service_task(void *pvParameters)
{
    ...
    while (1)
	{
        if(_get_async_event(&packet))
		{
            ...
            #ifdef i_printing_all_info_
            if(esp_task_wdt_add(NULL) != ESP_OK)
            {
              Serial.println("Failed to add async task to WDT");
            ...
            {
              Serial.println("Failed to remove loop task from WDT");
